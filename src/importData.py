...
def import_gene2pubmed(tax_id, input_file):
...
def run_import_gene2pubmed():
    """Function that runs import_gene2pubmed() while asking for input"""
    if len(sys.argv) == 1:
        ...
        import_gene2pubmed(tax_id, input_file)
        ...
        input_file = 'data/downloaded/gene2pubmed.gz'
        print(f"As only one argument was provided, input file is assumed to be data/downloaded/gene2pubmed.gz")
        import_gene2pubmed(tax_id, input_file)
        ...
        print(f"Usage: python {sys.argv[0]} tax_id(optional) dir_to_gene2pubmed.gz")
        sys.exit(1)
